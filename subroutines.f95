...
subroutine compute_total_energy()
	...
	real*8, dimension (n_lambda)	:: vij		! Pair potential energy
	...
	do i = 1, n_particles - 1
		...
		do j = i + 1, n_particles
			...
			! ***********************************************************************
			! Compute potential energy via spherical square-well (SW) force field
			! ***********************************************************************
			call compute_potential(modrij,vij)
			! ***********************************************************************
...
! *********************************************************************************************** !
subroutine compute_particle_energy(i,ri,vi)
	...
	real*8, dimension (3)		:: rij		! Vector distance between particles i and j
	real*8, dimension (n_lambda)	:: vi		! Potential energy of particle i (Iterative)
	real*8, dimension (n_lambda)	:: vij		! Pair potential energy
...
! *********************************************************************************************** !
subroutine compute_potential(modrij,vij)
...
subroutine check_overlap(i,ei,ri,overlap)
	...
	! ***************************************************************************************
	!  The loops below analyze the neighboring particles j around a fixed particle i
	!  and compute the contact distance (HGO) between i and j.
	! ***************************************************************************************
	...
	do j = 1, i - 1
		...
		if ( modrij <= cutoff ) then
			...
			urij(:) = rij(:) / modrij
			! ***********************************************************************
			! Contact distance - Hard Gaussian Overlap (HGO) model
			! See 'functions' file for more information
			! ***********************************************************************
			rhgo = sigmahgo(ei,emc(:,j),urij)
			! ***********************************************************************
			! Overlap criterion
			! ***********************************************************************
			if ( modrij <= rhgo ) then
...
subroutine block_averaging(flag,a1,a2,apert,dpa1,dpa2,dpapert)
	...
	! Subroutine log
	! ***************************************************************************************
	open ( UNIT = 100, FILE = "Perturbed_Coefficient/"//trim(descriptor_date)//"/Lambda_"//trim(descriptor_lamb)// &
	...
	! ***************************************************************************************
	open ( UNIT = 150, FILE = "Potential/"//trim(descriptor_date)//"/Lambda_"//trim(descriptor_lamb)// &
